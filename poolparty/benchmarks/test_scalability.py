...
from .timing import workload_complex_dag, workload_mutagenize, workload_recombine
...
class TestScaleNumSeqs:
    ...
    def test_mutagenize_num_seqs(self, num_seqs):
        result, elapsed = time_function(workload_mutagenize, seq_len=50, num_seqs=num_seqs)
    ...
    def test_complex_dag_num_seqs(self, num_seqs):
        result, elapsed = time_function(workload_complex_dag, seq_len=50, num_seqs=num_seqs)
        assert len(result) == num_seqs
...
class TestScaleSeqLength:
    ...
    def test_mutagenize_seq_length(self, seq_len):
        result, elapsed = time_function(workload_mutagenize, seq_len=seq_len, num_seqs=100)
    ...
    def test_recombine_seq_length(self, seq_len):
        result, elapsed = time_function(workload_recombine, seq_len=seq_len, num_seqs=100)
        assert len(result) == 100
...
class TestScaleLarge:
    ...
    def test_mutagenize_large_scale(self, num_seqs):
        result, elapsed = time_function(workload_mutagenize, seq_len=200, num_seqs=num_seqs)
        assert len(result) == num_seqs
        per_seq = elapsed / num_seqs * 1000
