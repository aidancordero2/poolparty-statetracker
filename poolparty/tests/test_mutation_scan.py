...
from poolparty.operations.mutagenize_using_num import MutagenizeUsingNumOp, mutagenize_using_num


class TestMutationScanFactory:
    """Test mutagenize_using_num factory function."""
    
    def test_returns_pool(self):
        """Test that mutagenize_using_num returns a Pool."""
        with pp.Party() as party:
            pool = mutagenize_using_num('ACGT', num_mutations=1)
            assert pool is not None
            assert hasattr(pool, 'operation')
    
    def test_creates_mutagenize_using_num_op(self):
        """Test that mutagenize_using_num creates a MutagenizeUsingNumOp."""
        with pp.Party() as party:
            pool = mutagenize_using_num('ACGT', num_mutations=1)
            assert isinstance(pool.operation, MutagenizeUsingNumOp)
    
    def test_accepts_string_input(self):
        """Test that mutagenize_using_num accepts string input."""
        with pp.Party() as party:
            pool = mutagenize_using_num('ACGT', num_mutations=1).named('mutant')
        
        df = pool.generate_seqs(num_seqs=1)
        assert len(df['seq'].iloc[0]) == 4
    
    def test_accepts_pool_input(self):
        """Test that mutagenize_using_num accepts Pool input."""
        with pp.Party() as party:
            seq = pp.from_seqs(['ACGT'])
            pool = mutagenize_using_num(seq, num_mutations=1).named('mutant')
...
class TestMutationScanSingleMutation:
    ...
    def test_single_mutation_count(self):
        ...
        with pp.Party() as party:
            pool = mutagenize_using_num('ACGT', num_mutations=1, mode='sequential').named('mutant')
    ...
    def test_single_mutation_preserves_length(self):
        ...
        with pp.Party() as party:
            pool = mutagenize_using_num('ACGTACGT', num_mutations=1, mode='sequential').named('mutant')
...
class TestMutationScanMultipleMutations:
    ...
    def test_double_mutation_count(self):
        ...
        with pp.Party() as party:
            pool = mutagenize_using_num('ACGT', num_mutations=2, mode='sequential').named('mutant')
    ...
    def test_triple_mutation(self):
        ...
        with pp.Party() as party:
            pool = mutagenize_using_num('ACGT', num_mutations=3, mode='sequential').named('mutant')
...
class TestMutationScanSequentialMode:
    ...
    def test_sequential_enumeration(self):
        ...
        with pp.Party() as party:
            pool = mutagenize_using_num('AC', num_mutations=1, mode='sequential').named('mutant')
...
class TestMutationScanRandomMode:
    ...
    def test_random_sampling(self):
        ...
        with pp.Party() as party:
            pool = mutagenize_using_num('ACGTACGT', num_mutations=1, mode='random').named('mutant')
    ...
    def test_random_num_states_is_one(self):
        ...
        with pp.Party() as party:
            pool = mutagenize_using_num('ACGT', num_mutations=1, mode='random')
...
class TestMutationScanAlphabets:
    ...
    def test_dna_alphabet(self):
        ...
        with pp.Party() as party:
            pool = mutagenize_using_num('ACGT', num_mutations=1, alphabet='dna', mode='sequential').named('mutant')
    ...
    def test_rna_alphabet(self):
        ...
        with pp.Party() as party:
            pool = mutagenize_using_num('ACGU', num_mutations=1, alphabet='rna', mode='sequential').named('mutant')
    ...
    def test_custom_alphabet(self):
        ...
        with pp.Party() as party:
            pool = mutagenize_using_num('AB', num_mutations=1, alphabet='AB', mode='sequential').named('mutant')
...
class TestMutationScanDesignCards:
    ...
    def test_positions_in_output(self):
        ...
        with pp.Party() as party:
            pool = mutagenize_using_num('ACGT', num_mutations=1, mode='sequential', op_name='mutate').named('mutant')
...
class TestMutationScanErrors:
    ...
    def test_k_zero_error(self):
        ...
        with pp.Party() as party:
            with pytest.raises(ValueError, match="num_mutations must be >= 1"):
                mutagenize_using_num('ACGT', num_mutations=0)
    
    def test_k_negative_error(self):
        """Test error for negative k."""
        with pp.Party() as party:
            with pytest.raises(ValueError, match="num_mutations must be >= 1"):
                mutagenize_using_num('ACGT', num_mutations=-1)
    ...
    def test_k_greater_than_length_error(self):
        ...
        with pytest.raises(ValueError, match="Cannot apply 3 mutations"):
            with pp.Party() as party:
                pool = mutagenize_using_num('AC', num_mutations=3, mode='sequential')
...
class TestMutationScanMutationMap:
    ...
    def test_all_mutations_covered(self):
        ...
        with pp.Party() as party:
            pool = mutagenize_using_num('A', num_mutations=1, alphabet='dna', mode='sequential').named('mutant')
...
class TestMutationScanCompute:
    ...
    def test_compute_sequential(self):
        ...
        with pp.Party() as party:
            pool = mutagenize_using_num('ACGT', num_mutations=1, mode='sequential')
...
class TestMutationScanWithParentPool:
    ...
    def test_with_sequential_parent(self):
        ...
        with pp.Party() as party:
            ...
            mutants = mutagenize_using_num(seqs, num_mutations=1, mode='sequential').named('mutant')
    ...
    def test_with_breakpoint_output(self):
        ...
        with pp.Party() as party:
            ...
            mutated_right = mutagenize_using_num(right, num_mutations=1, mode='sequential').named('mutant')
...
class TestMutationScanCustomName:
    ...
    def test_default_name(self):
        ...
        with pp.Party() as party:
            ...
            assert ':mutagenize_using_num' in pool.operation.name
    
    def test_custom_name(self):
        """Test custom operation name."""
        with pp.Party() as party:
            pool = mutagenize_using_num('ACGT', num_mutations=1, op_name='my_mutations')
    ...
    def test_custom_name_in_design_card(self):
        ...
        with pp.Party() as party:
            pool = mutagenize_using_num('ACGT', num_mutations=1, op_name='mutants').named('mypool')
...
class TestMutationScanNumStates:
    ...
    def test_num_states_k2(self):
        ...
        with pp.Party() as party:
            pool = mutagenize_using_num('ACGT', num_mutations=2, mode='sequential')
            # C(4,2) * 3^2 = 6 * 9 = 54
